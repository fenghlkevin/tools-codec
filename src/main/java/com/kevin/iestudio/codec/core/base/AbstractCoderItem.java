...
import com.kevin.iestudio.codec.core.annotation.CoderLinks;
import com.kevin.iestudio.codec.exception.CodecDecodeException;
import com.kevin.iestudio.codec.util.NTransUtil;
import com.kevin.iestudio.codec.util.ReflectUtil;
import com.kevin.iestudio.codec.util.StrHelper;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import com.kevin.iestudio.codec.core.annotation.IMapperConverter;
import com.kevin.iestudio.codec.exception.CodecBaseException;
...
public abstract class AbstractCoderItem implements Item {
			...
			for (; !thiz.equals(Object.class); thiz = thiz.getSuperclass()) {
				for (Field field : thiz.getDeclaredFields()) {
					if (field.isAnnotationPresent(CoderItem.class)) {
						...
						if (ei.type() != CoderItem.CoderItemType.LIST && !ReflectUtil.isInstanceOf(field, Item.class)) {
	...
	public int getEncodedMaxSize() {
		...
		for (String id : build) {
			...
			try {
				...
				if (ei.type() == CoderItem.CoderItemType.LIST) {
	...
	private void validate(Field field, CoderItem coderItem) {
		...
		if (coderItem.type() == CoderItem.CoderItemType.ITEM) {
			...
			// }
		} else if (coderItem.type() == CoderItem.CoderItemType.LIST) {
			// if (CoderItem.dependent() != null) {
			// throw new
			// TPEGBaseException("EncoderItemType List cannot Exist Dependent");
			// }
		} else if (coderItem.type() == CoderItem.CoderItemType.CRC) {
			if (coderItem.dependent() == null) {
		...
		} else if (coderItem.type() == CoderItem.CoderItemType.LENGTH) {
			if (coderItem.dependent() == null) {
		...
		} else if (coderItem.type() == CoderItem.CoderItemType.FLAGITEM_BITSWITCH || coderItem.type() == CoderItem.CoderItemType.FLAGITEM_SECLECTOR) {
			if (coderItem.dependent() == null) {
	...
	private void doCompile(String id, Map<String, byte[]> compile) throws IllegalArgumentException, IllegalAccessException {
		...
		byte[] temp = null;
		if (ei.type() == CoderItem.CoderItemType.ITEM) {
			temp = compileItem(field);
		} else if (ei.type() == CoderItem.CoderItemType.LIST) {
			temp = compileList(field, ei);
		} else if (ei.type() == CoderItem.CoderItemType.LENGTH) {
			temp = compileLength(field, ei, compile);
		} else if (ei.type() == CoderItem.CoderItemType.CRC) {
			temp = compileCRC(field, ei, compile);
		} else if (ei.type() == CoderItem.CoderItemType.FLAGITEM_BITSWITCH || ei.type() == CoderItem.CoderItemType.FLAGITEM_SECLECTOR) {
	...
	private Item newInstanceItem(Field field) {
		...
		if (ReflectUtil.isInstanceOf(field, NumberField.class)) {
			...
			if (ci.type() == CoderItem.CoderItemType.FLAGITEM_BITSWITCH) {
				item = new ByteFlag(ci.length(), ByteFlag.TYPE_BITSWITCH);
			} else if (ci.type() == CoderItem.CoderItemType.FLAGITEM_SECLECTOR) {
	...
	private boolean doDecompile(String id, InputStream stream, List<Field> selectors) throws IllegalArgumentException, IllegalAccessException, IOException {
		...
		if (ei.type() == CoderItem.CoderItemType.LIST) {
			...
			if (ei.type() == CoderItem.CoderItemType.FLAGITEM_BITSWITCH || ei.type() == CoderItem.CoderItemType.FLAGITEM_SECLECTOR) {
				selectors.add(field);
				thisIsFlag = true;
			}
			int bflag = -3;
	...
	public String toString() {
		...
		try {
			for (String id : build) {
				...
				if (ei.type() == CoderItem.CoderItemType.ITEM || ei.type() == CoderItem.CoderItemType.LENGTH || ei.type() == CoderItem.CoderItemType.CRC || ei.type() == CoderItem.CoderItemType.FLAGITEM_BITSWITCH
						|| ei.type() == CoderItem.CoderItemType.FLAGITEM_SECLECTOR) {
